...
import IFieldBookedQueue, {
  EBOOKED_QUEUE_STATUS,
} from "../interface/IFieldBookedQueue";
...
router.post("/", verifyToken, async (req: Request, res: Response) => {
  try {
    ...
    if (!footballshop)
      return res.status(404).json({ message: "FootballShop not found" });
    if (
      footballshop.status === EFootballShopStatus.PENDING ||
      footballshop.status === EFootballShopStatus.REJECTED
    )
      return res
        .status(400)
        .json({ message: "FootballShop is not approved yet" });
...
router.post("/book", verifyToken, async (req: Request, res: Response) => {
  try {
    ...
    if (booked_time <= new Date()) {
      return res
        .status(400)
        .json({ success: false, message: "Cannot book in the past" });
    }
    ...
    if (
      bookTime.getHours() < footballshop.active_at ||
      bookTime.getHours() > footballshop.inactive_at
    ) {
      return res.status(400).json({ message: "Cannot book at this time" });
    }

    const bookedQueue = await FieldBookedQueue.find({});
    const bookedQueueOfField = bookedQueue.filter(
      (f) => f.field_id?.toString() === field_id.toString()
    );

    for (const queue of bookedQueueOfField) {
      if (
        queue.booked_time?.getTime() === bookTime.getTime() ||
        queue.booked_time?.getTime() ===
          bookTime.getTime() + time_count * 3600000
      ) {
...
router.delete("/:id", verifyToken, async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: "Field deleted", data: field });
  } catch (error: any) {
    res.status(500).json({ success: false, message: error.message });
  }
});
